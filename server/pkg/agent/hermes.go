...
func newHermesBackend(cfg Config) *hermesBackend {
	return &hermesBackend{cfg: cfg}
}
...
func (b *hermesBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		if err != nil {
			...
			resCh <- Result{Status: finalStatus, Error: finalError, Provider: "hermes", DurationMs: time.Since(startTime).Milliseconds()}
			return
		}
		...
		if opts.ResumeSessionID != "" {
			_, err := c.request(runCtx, "session/resume", map[string]any{
				"cwd":       cwd,
